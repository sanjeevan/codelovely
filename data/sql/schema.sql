...
CREATE TABLE invite (id BIGINT AUTO_INCREMENT, code VARCHAR(100), user_id INT, status VARCHAR(255) DEFAULT 'unused', sent_at DATETIME, sent_to VARCHAR(255), invited_user_id INT, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, UNIQUE INDEX code_index_idx (code), INDEX user_id_idx (user_id), PRIMARY KEY(id)) ENGINE = INNODB;
